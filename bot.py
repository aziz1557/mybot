...
# Инициализация базовых полей пользователя для модерации
def init_user_fields(uid, name, username=""):
...
INSULT_PATTERNS = [
    ...
    r"\bпидарас\b", r"\bпидараса\b", r"\bпидор\b", r"\bпипора\b",
...
async def welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for member in update.message.new_chat_members:
        uid = str(member.id)
        init_user_fields(uid, member.first_name, member.username or "")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    init_user_fields(uid, user.first_name, user.username or "")
...
async def cmd_today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📅 Сегодня отличный день для продуктивных дел!")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 Привет! Я многофункциональный бот-модератор. Напиши команду /help, чтобы увидеть весь список моих возможностей.")
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "📊 <b>Информация:</b> /stats, /top, /info, /id, /chatinfo, /rules"
    )
